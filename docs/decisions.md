...
4. **`stg` and `main` branch protection turned on** (via the classic branch-protection API):
   `flow-guard`, `backend`, `frontend` required and must be up to date with the base before
   merging; force-pushes and branch deletion blocked; `enforce_admins` on, so the rule applies to
   the repo owner too — pushing directly to `stg`/`main` was already forbidden by convention
   (`docs/environments.md`), this makes it forbidden in practice. **No required PR-review count
   was set** — this is a solo project, and GitHub doesn't let an author approve their own PR, so
   requiring reviews would lock out every merge. `compose` was left out of the required checks:
   it verifies deploy config, not application correctness, and was never part of what blocks a
   promotion PR. Environment required-reviewers on `production` is still open — that Environment
   doesn't exist until Step 4 creates it.

**Consequences.** The git-history scan (decisions 1–3) is a one-time gate for *that* visibility
change, not a standing guarantee — a future commit could still introduce a secret; `.gitignore`'s
blanket `.env.*` exclusion remains the primary defence. Branch protection (decision 4) means a
promotion PR (`dev → stg`, `stg → main`) can no longer merge with a failing or stale check, and a
direct `git push` to either branch — including from the repo owner — is now rejected by GitHub
itself instead of only being a documented rule.
