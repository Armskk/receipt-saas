...
**Source of truth for what's next:** the Notion project board (task tracking is queued to move to GitHub Issues/Projects instead — see the backlog item for that). The checkboxes below are a technical shipped/not-shipped log for the deploy steps, kept here because they're detailed enough to be useful as reference — they are not a live backlog, so don't add new unchecked items here without also reflecting them in the actual tracker.
...
Hand-provisioning checklist for this step: `docs/production-provisioning-checklist.md`.
- [ ] GitHub Environment `production` + secrets (`PROD_SSH_HOST`/`PROD_SSH_USER`/`PROD_SSH_KEY`, variables `PROD_DEPLOY_PATH`/`PROD_API_DOMAIN`) — not yet created. The repo is public now (Step 5, done), so environment required-reviewers *is* available here, but `deploy-production.yml` still triggers only on a `v*` tag push or manual `workflow_dispatch` rather than every push to `main` — a deliberate extra gate for production, not a workaround for a plan limitation.
- [x] `.github/workflows/deploy-production.yml` — **done**: mirrors `deploy-stg.yml` (SSH in, checkout the triggering SHA, `docker compose -p receipt-prod … up -d --build --wait`, smoke-test `/health`) against the `receipt-prod` project and `.env.production`.
- [x] Backups — **done**: `scripts/backup.sh production` (see `docs/environments.md`'s "Backups" section above) dumps Postgres and mirrors the MinIO bucket to off-VM storage via `rclone`. Still to do by hand: configure the `rclone` remote (OCI Object Storage), schedule the cron job, and run one test restore — all in `docs/production-provisioning-checklist.md`.
- [x] Rollback procedure — **done**: documented in `docs/environments.md`'s "Rollback" section above (re-run the deploy workflow against an older ref; expand/contract for schema changes, since migrations are forward-only).
- [x] Monitoring — **done** (as documentation + one script): `docs/environments.md`'s "Monitoring" section above covers `/health` uptime checks, container-restart and disk-space alerting, and reviewing Claude spend via `usage_logs`. Still to do by hand: actually point an external uptime monitor at `/health` (`docs/production-provisioning-checklist.md`).
...
- [ ] Make `flow-guard`, `backend` and `frontend` required status checks, protect `stg`/`main`, enable environment required-reviewers on `production` — available for free now that the repo is public; not yet configured.
